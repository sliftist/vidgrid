...
export interface Palette {
    ...
    gridTextColor?: string;  // color override for GridCell title (defaults to text)
    gridTextShadow?: string; // text-shadow outline on GridCell title + info (neon themes)
...
export function buildTheme(p: Palette): string {
    ...
    const gridTextShadow = p.gridTextShadow ? `text-shadow: ${p.gridTextShadow};` : "";
...
.GridCell-title { color: ${p.gridTextColor || p.text}; ${gridTextShadow} }
.GridCell-info { color: ${p.gridTextColor || p.muted}; ${gridTextShadow} }
...
export const VAPORWAVE_CSS = buildTheme({
    ...
    gridTextColor: "hsl(0, 0%, 100%)",
    gridTextShadow: "1px 1px 0 hsl(263, 67%, 9%), -1px 1px 0 hsl(263, 67%, 9%), 1px -1px 0 hsl(263, 67%, 9%), -1px -1px 0 hsl(263, 67%, 9%)",
