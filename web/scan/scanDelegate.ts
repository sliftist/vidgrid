...
import type { ExtractedFrame } from "./MetadataExtractorClient";
...
function send<T>(op: string, relativePath: string, extra: Record<string, unknown>, onFrame?: (f: ExtractedFrame) => Promise<void> | void): Promise<T> {
    return new Promise<T>((resolve, reject) => {
        if (!victimPort) { reject(new Error("no decode victim available")); return; }
        const reqId = ++reqCounter;
        pending.set(reqId, { resolve, reject, onFrame });
        try {
...
export const remoteExtractor = {
    ...
    extractKeyframes(relativePath: string, _label: string, softwareDecode: boolean): Promise<KeyframeBundle> {
        return send<KeyframeBundle>("extractKeyframes", relativePath, { softwareDecode });
    },
    extractFaceFrames(relativePath: string, _label: string, onFrame: (f: ExtractedFrame) => Promise<void> | void, fp16: boolean, softwareDecode: boolean): Promise<number> {
        return send<number>("extractFaceFrames", relativePath, { fp16, softwareDecode }, onFrame);
    },
    abort(): void {
